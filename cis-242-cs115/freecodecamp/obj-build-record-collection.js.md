...
## O(n)
Correct. Your `for...of` with `Object.entries()` iterates every key to find a match — **O(n)**. Direct property access `my[id]` is a hash table lookup — **O(1)**.

One nuance worth mentioning for your students: `Object.entries()` also **creates a new array** of all key-value pairs before the loop even starts, so there's an extra O(n) memory cost on top of the O(n) time.

## The exercise found in freecodecamp object: build a record collection
