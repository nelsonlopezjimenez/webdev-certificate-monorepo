...
Write a function called **`includes`** which accepts a `array`, a `value`, and an optional starting `index`. The function should return `true` if the `value` exists in the array when we search starting from the starting `index`. Otherwise, it should return `false`.
